"""
SparkVSR Service Module
Handles SparkVSR processing logic, presets, and callbacks
"""
...
def run_sparkvsr(*args, **kwargs):
    ...
    from shared.sparkvsr_runner import run_sparkvsr as _run_sparkvsr_impl
...
def _move_or_convert_image_output(
    ...
    Used by SparkVSR batch-image processing so the public output folder contains
    only image files named like the inputs, even though SparkVSR internally
...
def _safe_ui_video_preview_path(video_path: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        st = src.stat()
        sig = hashlib.sha256(
            f"{src.as_posix()}|{st.st_size}|{st.st_mtime_ns}|SparkVSR_ui_preview_v1".encode("utf-8")
        ).hexdigest()[:20]
...
def _nearest_supported_scale(value: Any, default: int = 4) -> int:
    ...
    return max(1, min(16, int(round(raw))))


def canonical_sparkvsr_scale(
    *,
    ...
    """
    Resolve SparkVSR scale to a backend-supported integer factor.
...
def sparkvsr_defaults(model_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    default_model = model_name or get_sparkvsr_default_model()
    ...
    if model_meta:
        ...
        tile_height = int(model_meta.default_tile_height)
        tile_width = int(model_meta.default_tile_width)
        overlap_height = int(model_meta.default_overlap_height)
        overlap_width = int(model_meta.default_overlap_width)
        model_path = ""
    else:
...
def _sparkvsr_dict_from_args(args: List[Any]) -> Dict[str, Any]:
    return dict(zip(SPARKVSR_ORDER, args))


def _enforce_sparkvsr_guardrails(cfg: Dict[str, Any], defaults: Dict[str, Any]) -> Dict[str, Any]:
    cfg = {**defaults, **(cfg or {})}
    ...
    try:
        from shared.models.sparkvsr_meta import get_sparkvsr_model_names

        valid_model_names = set(get_sparkvsr_model_names())
    except Exception:
        valid_model_names = {str(defaults.get("model_name", "SparkVSR-S2"))}
    model_name = str(cfg.get("model_name", defaults.get("model_name", "SparkVSR-S2")) or "SparkVSR-S2").strip()
    if valid_model_names and model_name not in valid_model_names:
        model_name = str(defaults.get("model_name", "SparkVSR-S2") or "SparkVSR-S2")
    cfg["model_name"] = model_name
    cfg["model_path"] = str(cfg.get("model_path", defaults.get("model_path", "")) or "").strip()
    cfg["lora_path"] = str(cfg.get("lora_path", defaults.get("lora_path", "")) or "").strip()

    scale = canonical_sparkvsr_scale(
    ...
    cfg["tile_height"] = max(0, min(4096, _to_int(cfg.get("tile_height"), 0)))
    cfg["tile_width"] = max(0, min(4096, _to_int(cfg.get("tile_width"), 0)))
    if (cfg["tile_height"] <= 0) != (cfg["tile_width"] <= 0):
        cfg["tile_height"] = 0
        cfg["tile_width"] = 0
    ...
    cfg["pisa_gpu"] = str(cfg.get("pisa_gpu", defaults.get("pisa_gpu", "0")) or "0").strip()
    cfg["png_save"] = _to_bool(cfg.get("png_save"), _to_bool(defaults.get("png_save", False), False))
    save_format = str(cfg.get("save_format", defaults.get("save_format", "yuv444p")) or "yuv444p").strip().lower()
    cfg["save_format"] = save_format if save_format in set(SPARKVSR_SAVE_FORMAT_OPTIONS) else "yuv444p"
    ...
    if "," in device_str:
        ...
        cfg["_multi_gpu_disabled_reason"] = "SparkVSR is single-GPU."
    else:
        cfg["device"] = device_str or "auto"

    # Compatibility aliases for older presets/components.
    cfg["dtype"] = cfg.get("precision", "bfloat16")
    cfg["quality"] = cfg.get("crf", 18)

    return cfg


def _apply_sparkvsr_preset(
    preset: Dict[str, Any],
    ...
    merged = _enforce_sparkvsr_guardrails(merged, defaults)
    return [merged[k] for k in SPARKVSR_ORDER]


def build_sparkvsr_callbacks(
    preset_manager: PresetManager,
    runner,
    ...
    def refresh_presets(model_name: str, select_name: Optional[str] = None):
        ...
        presets = preset_manager.list_presets("sparkvsr", model_name)
        last_used = preset_manager.get_last_used_name("sparkvsr", model_name)
    ...
    def save_preset(preset_name: str, *args):
        ...
        try:
            payload = _sparkvsr_dict_from_args(list(args))
            model_name = str(payload.get("model_name") or get_sparkvsr_default_model())
            
            preset_manager.save_preset_safe("sparkvsr", model_name, preset_name.strip(), payload)
            dropdown = refresh_presets(model_name, select_name=preset_name.strip())

            current_map = dict(zip(SPARKVSR_ORDER, list(args)))
            loaded_vals = _apply_sparkvsr_preset(payload, defaults, preset_manager, current=current_map)
    ...
    def load_preset(preset_name: str, model_name: str, current_values: List[Any]):
        ...
        """
        try:
            model_name = str(model_name or get_sparkvsr_default_model())
            preset = preset_manager.load_preset_safe("sparkvsr", model_name, preset_name)
            if preset:
                preset_manager.set_last_used("sparkvsr", model_name, preset_name)

            current_map = dict(zip(SPARKVSR_ORDER, current_values))
            values = _apply_sparkvsr_preset(preset or {}, defaults, preset_manager, current=current_map)
    ...
    def safe_defaults():
        ...
        normalized = _enforce_sparkvsr_guardrails(defaults.copy(), defaults)
        return [normalized[key] for key in SPARKVSR_ORDER]
    ...
    def run_action(
        ...
        *args,
        ...
        try:
            ...
            seed_controls["sparkvsr_chunk_preview"] = {
                "message": "No chunk preview available yet.",
                "gallery": [],
                "videos": [],
                "count": 0,
            }
            seed_controls["sparkvsr_batch_outputs"] = []
            state["seed_controls"] = seed_controls
            settings_dict = _sparkvsr_dict_from_args(list(args))
            ...
            # Apply SparkVSR guardrails (single GPU, tile validation, etc.)
            settings = _enforce_sparkvsr_guardrails(settings, defaults)
            if preview_only:
                settings["batch_enable"] = False
            use_global_resolution = bool(settings.get("use_resolution_tab", True))
            
            # Apply shared Resolution tab scale with SparkVSR backend limits (2x or 4x only).
            if use_global_resolution:
                if seed_controls.get("upscale_factor_val") is not None:
                    try:
                        ...
                        fixed_scale = canonical_sparkvsr_scale(
                        ...
                        if abs(global_scale - float(fixed_scale)) > 0.01:
                            settings["_scale_clamp_note"] = (
                                f"Resolution tab requested {global_scale:.2f}x; SparkVSR uses integer upscale factors, "
                                f"so {fixed_scale}x will be used."
                            )
                    except Exception:
                        pass
                # Max-edge cap and pre-downscale toggle are local to SparkVSR tab.
            ...
            # SparkVSR tab uses global Output tab video settings for model-pass encoding.
            ...
            if output_codec_key and output_codec_key not in codec_map:
                settings["_output_codec_note"] = (
                    f"Output tab codec '{output_codec_key}' is not supported by SparkVSR CLI writer; "
            ...
            # Clear cancel event
            _sparkvsr_cancel_event.clear()

            def _apply_vnext_preprocess(cfg: Dict[str, Any], src_input_path: str) -> None:
                """
                vNext preprocessing for fixed-scale SparkVSR runs.
                ...
                try:
                    ...
                    if in_type == "video":
                        ...
                        if progress:
                            progress(0, desc=f"Preprocessing input -> {int(plan.preprocess_width)}x{int(plan.preprocess_height)}")
                        ok, _err = scale_video(
                            Path(src_input_path),
                            Path(pre_out),
                    ...
                    if in_type == "image":
                        ...
                        if progress:
                            progress(0, desc=f"Preprocessing image -> {int(plan.preprocess_width)}x{int(plan.preprocess_height)}")
                    ...
                    if in_type == "directory":
                        ...
                        if progress:
                            progress(0, desc=f"Preprocessing frames -> {int(plan.preprocess_width)}x{int(plan.preprocess_height)}")
            ...
            # Initialize progress
            if progress:
                progress(0, desc="Initializing SparkVSR...")
            ...
            def _cache_chunk_preview(run_dir: Optional[Path]) -> None:
                try:
                    if not run_dir:
                        ...
                        seed_controls["sparkvsr_chunk_preview"] = build_chunk_preview_payload(str(run_dir))
                    state["seed_controls"] = seed_controls
                except Exception:
                    pass
            ...
            # -------------------------------------------------------------
            # [OK] Batch processing (folder of videos and/or frame directories)
            # -------------------------------------------------------------
            if bool(settings.get("batch_enable")):
                ...
                if not items:
                    vid_upd, img_upd = _media_updates(None)
                    yield (
                        "[ERROR] No media files found in batch input folder",
                ...
                # Universal chunking (Resolution tab) applies to SparkVSR batch runs too.
                ...
                def _batch_live_payload(status_text: str, preview_out: Optional[str] = None):
                    seed_controls["sparkvsr_batch_outputs"] = list(outputs)
                    state["seed_controls"] = seed_controls
                ...
                def _cleanup_batch_image_work_dir(cfg: Dict[str, Any]) -> None:
                    image_work_dir_raw = cfg.get("_sparkvsr_batch_image_work_dir") if isinstance(cfg, dict) else None
                    ...
                    try:
                        ...
                        if work_parent.name == ".sparkvsr_image_work":
                            try:
                                work_parent.rmdir()
                            except Exception:
                                pass
                ...
                yield _batch_live_payload(f"Starting SparkVSR batch ({len(items)} item(s))")
                ...
                for idx, item in enumerate(items, 1):
                    ...
                    if is_batch_image_item:
                        ...
                        image_work_dir = batch_root / ".sparkvsr_image_work" / safe_stem
                        try:
                        ...
                        item_settings["_sparkvsr_batch_image_desired_output"] = str(desired_output_file)
                        item_settings["_sparkvsr_batch_image_work_dir"] = str(image_work_dir)
                    ...
                    if not fps_pre_ok:
                        logs.append(
                            f"[ERROR] [{idx}/{len(items)}] {Path(item_path).name} failed: "
                    ...
                    if should_chunk_video:
                        ...
                        class _CancelProbe:
                            def is_canceled(self) -> bool:
                                return bool(_sparkvsr_cancel_event.is_set())
                    ...
                    if outp and Path(outp).exists():
                        ...
                        batch_image_desired = item_settings.get("_sparkvsr_batch_image_desired_output")
                        ...
                        if Path(outp).suffix.lower() in video_exts:
                            ...
                            if rife_out and Path(rife_out).exists():
                                logs.append(f"[OK] Global RIFE output: {Path(rife_out).name}")
                                outp = rife_out
                            elif rife_msg:
                                logs.append(f"[WARN] {rife_msg}")
                            comp_vid_path, comp_vid_err = maybe_generate_input_vs_output_comparison(
                                ...
                                outp,
                                seed_controls,
                                label_output="sparkvsr",
                                on_progress=None,
                            )
                            if comp_vid_path:
                                logs.append(f"[OK] Comparison video created: {Path(comp_vid_path).name}")
                            elif comp_vid_err:
                                logs.append(f"[WARN] Comparison video failed: {comp_vid_err}")
                        ...
                        if chunk_count_item:
                            logs.append(f"[OK] [{idx}/{len(items)}] {Path(item_path).name} -> {Path(outp).name} ({int(chunk_count_item)} chunks)")
                        else:
                            logs.append(f"[OK] [{idx}/{len(items)}] {Path(item_path).name} -> {Path(outp).name}")
                    else:
                        if getattr(result, "returncode", 0) != 0:
                            maybe_set_vram_oom_alert(state, model_label="sparkvsr", text=getattr(result, "log", ""), settings=item_settings)
                ...
                status = f"[OK] SparkVSR batch complete ({len(outputs)}/{len(items)} succeeded, {len(items) - len(outputs)} failed)"
            ...
            if settings.get("_preprocess_required_but_missing"):
                vid_upd, img_upd = _media_updates(None)
                yield (
                    "[ERROR] Max resolution preprocess failed",
                    str(
                        settings.get("_preprocess_error_note")
                        or "Pre-downscale was required but failed, so SparkVSR run was stopped to avoid uncapped output."
                    ),
                    vid_upd,
                    img_upd,
                    gr.update(visible=False),
                        ...
                        def _chunk_progress_cb(progress_val, desc="", **kwargs):
                            ...
                            if str(kwargs.get("phase", "")).strip().lower() == "completed":
                                try:
                                    ...
                                    seed_controls["sparkvsr_chunk_preview"] = build_chunk_preview_payload(str(run_root))
            ...
            def _normalize_live_progress_line(msg_text: str) -> Tuple[str, bool]:
                ...
                if tile_match:
                    ...
                    if pct_match is not None:
                        tile_pct = max(0.0, min(100.0, float(pct_match.group(1))))
                    else:
                        tile_pct = (float(max(0, tile_idx - 1)) / float(tile_total)) * 100.0
                    return f"Processing Tiles: {tile_idx}/{tile_total} ({tile_pct:.1f}%)", True

                text_lc = text.lower()
                if text_lc.startswith("[SparkVSR] processing..."):
                    return text, True
                if text_lc.startswith("[sparkvsr] still running"):
                    return text, True
                if "loading pipeline components" in text_lc or "loading checkpoint shards" in text_lc:
                    return text, True
            ...
            def _extract_progress(msg_text: str) -> Tuple[Optional[float], str]:
                ...
                if "loading pipeline components" in text.lower() or "loading checkpoint shards" in text.lower():
                    m = re.search(r"(\d+(?:\.\d+)?)\s*%", text)
                    if m:
                        try:
                            raw = max(0.0, min(100.0, float(m.group(1)))) / 100.0
                            pct = 0.01 + (raw * 0.09)
                        except Exception:
                            pct = None
                    return pct, text
                ...
                tile_match = re.search(
                    r"Processing\s+Tiles:\s*(\d+)\s*/\s*(\d+).*?\((\d+(?:\.\d+)?)%\)",
                    text,
                    flags=re.IGNORECASE,
                )
                if tile_match:
                    try:
                        pct = max(0.0, min(100.0, float(tile_match.group(3)))) / 100.0
                    except Exception:
                        pct = None
                    return pct, text

                m = re.search(r"Processing\s+Tiles:\s*(\d+)\s*/\s*(\d+)", text, flags=re.IGNORECASE)
                if m:
                    try:
                ...
                m = re.search(r"(?:Processed|Processing):\s*(\d+)\s*/\s*(\d+)", text, flags=re.IGNORECASE)
                if m:
                    try:
                        num = int(m.group(1))
                        den = max(1, int(m.group(2)))
            ...
            while thread.is_alive() or not progress_queue.empty():
                    ...
                    if temp_chunks_dir.exists():
                        try:
                            ...
                            if completed_chunks and len(completed_chunks) > 0:
                                partial_target = _collision_safe_path_local(temp_chunks_dir / "cancelled_SparkVSR_partial.mp4")
                                if concat_videos(completed_chunks, partial_target, encode_settings=settings):
                                    final_output = Path(output_dir) / f"cancelled_SparkVSR_partial_upscaled.mp4"
                ...
                try:
                    ...
                    if msg_clean:
                        ...
                        has_real_hint = (
                            ...
                            or "speed:" in msg_lc
                            or "s/iter" in msg_lc
                            or msg_lc.startswith("[sparkvsr] still running")
                        )
                        if has_real_hint:
                ...
                if now - last_update > 0.5:
                    ...
                    if live_progress_desc:
                        ...
                        status_live = f"[RUNNING] SparkVSR processing... {int(live_progress_pct * 100)}% ({elapsed_s}s)"
                    else:
                        status_live = f"[RUNNING] SparkVSR processing... {elapsed_s}s elapsed"
                    yield (
                        status_live,
                        "\n".join(log_buffer[-50:]),
                        vid_upd,
            ...
            if "error" in result_holder:
                ...
                if maybe_set_vram_oom_alert(state, model_label="sparkvsr", text=result_holder.get("error", ""), settings=settings):
                    show_vram_oom_modal(state, title="Out of VRAM (GPU) - SparkVSR", duration=None)
                yield (
                    ("[OOM] Out of VRAM (GPU) - see banner above" if state.get("alerts", {}).get("oom", {}).get("visible") else "[ERROR] Processing failed"),
            ...
            if progress:
                progress(1.0, desc="SparkVSR complete!")
            ...
            if face_apply and output_path and Path(output_path).exists():
                ...
                if Path(output_path).suffix.lower() in ['.mp4', '.avi', '.mov', '.mkv']:
                    ...
                    if restored and Path(restored).exists():
                        ...
                        log_buffer.append(f"[OK] Face restoration complete: {restored}")
                else:
                    # Image restoration
                    restored_img = restore_image(
                        output_path,
                    ...
                    if restored_img and Path(restored_img).exists():
                        ...
                        log_buffer.append(f"[OK] Face restoration complete: {restored_img}")
            ...
            if chunk_count > 0:
                status = (
                    f"[OK] SparkVSR chunked upscale complete ({chunk_count} chunks)"
                    if result.returncode == 0
                    else f"[WARN] Chunked upscale exited with code {result.returncode}"
                )
            else:
                status = "[OK] SparkVSR upscaling complete" if result.returncode == 0 else f"[WARN] Exited with code {result.returncode}"
            if preview_only:
                if result.returncode == 0:
                    status = "Preview complete"
                elif _sparkvsr_cancel_event.is_set():
                    status = "Preview canceled"
                else:
                    status = "Preview failed"

            if result.returncode != 0 and maybe_set_vram_oom_alert(state, model_label="sparkvsr", text=result.log, settings=settings):
            ...
            if maybe_set_vram_oom_alert(state, model_label="sparkvsr", text=str(e), settings=locals().get("settings")):
                show_vram_oom_modal(state, title="Out of VRAM (GPU) - SparkVSR", duration=None)
            yield (
    ...
    def cancel_action():
        """Cancel SparkVSR processing"""
        _sparkvsr_cancel_event.set()
        return gr.update(value="[STOP] Cancellation requested - SparkVSR will stop at next checkpoint"), "Cancelling..."

    def open_outputs_folder_SparkVSR():
    ...
    def clear_temp_folder_SparkVSR(confirm: bool):
    ...
    return {
        ...
        "open_outputs_folder": open_outputs_folder_SparkVSR,
        "clear_temp_folder": clear_temp_folder_SparkVSR,
    }
