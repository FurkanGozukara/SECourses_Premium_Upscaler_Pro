...
def _check_universal_preset_caches() -> None:
    ...
    res_vals[RESOLUTION_ORDER.index("scene_threshold")] = 31.0
    res_vals[RESOLUTION_ORDER.index("min_scene_len")] = 2.0
    sync_tab_to_shared_state("resolution", res_vals, state2)
    assert float(state2["seed_controls"]["resolution_settings"]["scene_threshold"]) == 31.0
    assert float(state2["seed_controls"]["resolution_settings"]["min_scene_len"]) == 2.0
    ...
    assert float(state2["seed_controls"].get("scene_threshold", 0.0) or 0.0) == 31.0
    assert float(state2["seed_controls"].get("min_scene_len", 0.0) or 0.0) == 2.0
